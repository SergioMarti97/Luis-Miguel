...
Se van a eliminar los datos del paciente 88. Aunque no haya ningún problema con sus datos clínicos, los datos de expresión genética no se pueden analizar con los demás miembros devido a un error en el laboratorio.
...
lSamples <- dfClinical$Samples
lSamples <- lSamples[!lSamples %in% c(63, 64, 113, 188)] 
lSurvival <- na.omit(as.numeric(dfClinical$Survival))
names(lSurvival) <- lSamples

# Transformar con Log10
lLogSurvival <- log10(lSurvival)

mLogSurvival <- as.matrix(lLogSurvival)

distSurvival <- dist(mLogSurvival)

hcSurvival <- hclust(distSurvival, method = "complete")

fSurvival <- cutree(hcSurvival, k = 3)

fSurvival <- factor(fSurvival)

levels(fSurvival) <- c("media", "alta", "baja")

fSurvival

# tabla
t1 <- as.data.frame(table(fSurvival))
colnames(t1) <- c("cluster", "Freq")
t1 <- tableGrob(t1)

# boxplot
g1 <- ggplot(data.frame(weeks = lSurvival, cluster = fSurvival), 
  aes(x = reorder(cluster, weeks), y = log10(weeks), fill = cluster)) +
  stat_boxplot(geom = "errorbar", width = 0.25) + 
  xlab("cluster") +
  ylab("log10(Survival)") + 
  geom_boxplot() + 
...
# dendograma
hcplot <- ggdendro::ggdendrogram(hcSurvival, rotate = TRUE) + 
...
gQQ1 <- ggplot(dfClinical[dfClinical$Survival != "N.D." & dfClinical$fTumorHemisphere == "left",], aes(sample = log10(as.numeric(Survival)))) + 
  ...
  theme(axis.title =element_text(size = 9), plot.title = element_text(size=9)) +
  ylab("log10(Weeks)") + 
  ggtitle("QQplot left hemisphere")
...
gQQ2 <- ggplot(dfClinical[dfClinical$Survival != "N.D." & dfClinical$fTumorHemisphere == "right",], aes(sample = log10(as.numeric(Survival)))) + 
  ...
  theme(axis.title = element_text(size = 9), plot.title = element_text(size=9)) +
  ylab("log10(Weeks)") + 
  ggtitle("QQplot right hemisphere")
